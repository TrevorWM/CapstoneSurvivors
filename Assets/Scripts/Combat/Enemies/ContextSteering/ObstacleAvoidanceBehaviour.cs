...
{
    // radius is for obstacle detection, collider size is size of enemy
    [SerializeField]
    ...
    // returns anonymous structures so we dont have to create new structures or classes
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
    {
        // check all detected obstacles
        foreach (Collider2D obstacleCollider in aiData.obstacles)
        {
            ...
            {
                // gets approx result of which direction not to move in
